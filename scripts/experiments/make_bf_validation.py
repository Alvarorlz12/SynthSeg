...
The pipeline reproduces the generator's, not an approximation of it:
  ...
  read (predict_tm.preprocess with predict_bf's arguments: 1 mm, 160 crop centred on the volume,
        min_pad = crop, p0.5-p99.5 after the crop)
...
Two things that are easy to get wrong:
  ...
  near 0.43. With --align_corners it is lower still (no voxel copies a value whole, all lie between
  two), so a ladder for those fields goes up to the sigma that run trained with.
  ...
  # the ladder for the align_corners runs: one scale per image, in turn
  python scripts/experiments/make_bf_validation.py \
      --images <data>/qc-data/validation/img \
      --out_img <data>/qc-data/validation/img_bf_corners \
      --out_csv <data>/qc-data/validation/gt/bf_corners/targets.csv \
      --ladder 0 0.225 0.45 0.675 0.9 --bias_scale 0.0125 0.01875 0.025 --align_corners
...
class FixedBiasField(layers.BiasFieldCorruption):
    ...
    def __init__(self, sigma, seed, bias_scale=0.025, align_corners=False, **kwargs):
        ...
        super().__init__(bias_field_std=1.0, bias_scale=bias_scale, same_bias_for_all_channels=False,
                         prob=1.0, return_field=True, align_corners=align_corners, **kwargs)
    ...
    def call(self, inputs, **kwargs):
        ...
        log_bias = nrn_layers.Resize(size=self.inshape[0][1:self.n_dims + 1], interp_method='linear',
                                     align_corners=self.align_corners)(unit * self.sigma)
...
def build_corrupter(image_shape, sigma, seed, bias_scale, bias_field_std=0.7, bias_prob=0.9,
                    sample=False, align_corners=False):
    ...
    if sample:
        img, log_bias = layers.BiasFieldCorruption(bias_field_std, bias_scale, False, prob=bias_prob,
                                                   return_field=True, align_corners=align_corners)(inp)
    else:
        img, log_bias = FixedBiasField(sigma, seed, bias_scale, align_corners)(inp)
...
def main():
    ...
    p.add_argument('--bias_scale', type=float, nargs='+', default=[0.025],
                   help='several values: in --sample the layer draws one per image; in the ladder image i '
                        'gets value i mod n, one field shape per image as always')
    p.add_argument('--align_corners', action='store_true',
                   help='end values of the small field on the end voxels of each axis (the layer option)')
    ...
    for i, path in enumerate(paths):
        ...
        im, _, _, _, _, _, _, _ = preprocess(path_image=path, n_levels=a.n_levels,
                                             target_res=a.target_res, crop=a.cropping, min_pad=a.cropping)
        shape = list(im.shape[1:])
        # one seed per anatomy: the shape of the field is that image's own
        seed = a.seed + 1000 * i
        scale = a.bias_scale if a.sample else a.bias_scale[i % len(a.bias_scale)]
        ...
        for k, sigma in rungs:
            ...
            out, std = build_corrupter(shape, sigma, seed, scale, a.bias_field_std,
                                       a.bias_prob, a.sample, a.align_corners).predict(im)
            ...
            rows.append((name, stem, k, '' if a.sample else '%d' % seed,
                         ...
                         std, ' '.join('%g' % s for s in utils.reformat_to_list(scale)), a.cropping,
                         a.target_res, int(a.align_corners)))
            print('  %-40s b_std %-6s -> std(log B) %.4f'
                  % (name, '?' if sigma is None else '%.2f' % sigma, std))

    with open(a.out_csv, 'w') as fh:
        fh.write('stem,source,level,seed,b_std,std_log_bias,bias_scale,cropping,target_res,align_corners\n')
        for r in rows:
            fh.write('%s,%s,%d,%s,%s,%.6f,%s,%d,%.2f,%d\n' % r)
