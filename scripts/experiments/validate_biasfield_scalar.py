...
def build_validation_set(a, gen_labels, gen_classes, hold_paths, cache_path):
    ...
    for i in range(a.n_images):
        ...
        images.append(img[0]); tgt.append(y[0])
    ...
    if cache_path is not None:
        np.savez(cache_path, images=images, target=tgt)
        print('  cached the validation set at %s (%.2f GB)' % (cache_path, images.nbytes / 1e9))
    return images, tgt
...
def validate_training(a, model_dir, validation_dir, images, target):
    ...
    ckpts = sorted(glob.glob(os.path.join(model_dir, 'bf_*.h5')), key=epoch_of)[::a.step_eval]
