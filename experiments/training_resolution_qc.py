...
Older per-axis resolution regressor (the v5/v6 lineage), superseded by
QC/training_rs.py and kept for reference: this is not the head that is trained
or deployed.

labels_to_image_model(randomise_res=True, return_resolution=True) degrades the GMM image to a random
per-axis spacing and exposes it as the layer 'resolution'; per axis, directional roughness features
(mean-squared 1st/2nd finite differences) feed one weight-shared Dense head; the weighted log-Huber
loss is computed in-graph and returned as the model output, so it compiles with metrics.IdentityLoss().
Training goes through train_model (training_qc.py), which saves a checkpoint every epoch.
...
# project imports
from SynthSeg.training_qc import train_model          # reused verbatim
...
def training(labels_dir,
             ...
             spectral=False,
             rolloff=False,
             # drop the absolute log-raw energies lg1/lg2 from the descriptor
             drop_abs=False,
             n_levels=5,
             nb_conv_per_level=2,
             conv_size=3,
    ...
    :param output_labels: (optional) does not affect the resolution target. Defaults to generation_labels.
    ...
    # head architecture
    :param no_context: (optional) if True (default) the head is the directional roughness into a shared
    Dense; if False, a down-projected conv-encoder context is concatenated to it.
    ...
    :param spectral: (optional) append the per-axis 1D-spectrum shape features. Default False.
    :param rolloff: (optional) append the per-axis spectral roll-off feature. Default False.
    :param drop_abs: (optional) drop the absolute log-raw energies from the descriptor. Default False.
    ...
    # 1) generator graph: synthesis + the in-graph per-axis spacing, exposed as the layer 'resolution'
    ...
    # trainable size of the regressor (the generator layers hold no trainable weights)
...
# directional roughness features, per axis
...
def _grad_energy(image, order, n_dims=3):
    """Mean-squared finite difference (order 1 or 2) along each spatial axis, [B, n_dims]. A coarser
    axis is smoother along its own direction, so this carries the cutoff."""
    es = []
...
def build_directional_features(image, n_dims=3, spectral=False, rolloff=False, drop_abs=False):
    """Per-axis directional descriptor [B, n_dims, F_dir]: cross-axis-normalized 1st/2nd difference
    energies and, unless drop_abs, their log raw energies. spectral and rolloff append the spectral
    features. 2 features with drop_abs, 4 by default, up to 8 with everything on."""
    ...
    base = [g1n, g2n]                                                              # relative backbone
    if not drop_abs:
        base.append(KL.Lambda(lambda t: K.log(t + K.epsilon()), name='g1_log')(g1))   # log raw energy
    ...
    extra = []
    if spectral:
        extra.append(build_spectral_features(image, n_dims))                       # [B, nd, 3]
    if rolloff:
        extra.append(build_rolloff_features(image, n_dims))                        # [B, nd, 1]
    if not extra:
        return fd                                                                  # 4 feats, or 2 with drop_abs
    return KL.Concatenate(axis=-1, name='dir_feat_cat')([fd] + extra)              # 4+1=5, 4+3=7, or 2+1=3


# per-axis 1D spectral shape features
def _axis_power(image, ax, tag='spec'):
    """Mean 1D power spectrum along spatial axis `ax` (1,2,3) of [B,X,Y,Z,1], averaged over the other
    two spatial axes, [B, F] with F = N_ax//2 + 1 (real FFT)."""
    def fn(t):
        x = t[..., 0]                                              # [B, X, Y, Z]
...
def _spectral_summaries(p):
    """[B, F] power spectrum to [B, 3] shape features in normalized frequency, DC dropped: spectral
    centroid, normalized entropy, and log(high-band / low-band) energy ratio, split at half-Nyquist."""
    def fn(p):
        eps = K.epsilon()
        F = tf.shape(p)[-1]
...
# per-axis cumulative-energy spectral roll-off
def _axis_rolloff(image, ax, frac=0.95):
    """Cumulative-energy spectral roll-off along spatial axis `ax` (1,2,3), giving [B, 1]: the normalized
    frequency below which `frac` (default 95%) of that axis's spectral energy lies, DC dropped. It reads
    the per-axis detail bandwidth rather than its magnitude."""
...
def build_resqc_model(generator, n_dims,
    ...
    image = generator.outputs[0]                                  # [B, X, Y, Z, 1], min-max normalized
    #                                                               before the degradation, not after
    ...
    if no_context:
        ...
        # shared conv encoder giving global context, no decoder. batch_norm=None: at batchsize 1, BN
        # normalizes away the spatial-amplitude statistic read here.
        ...
        # down-project the context so it cannot drown the directional features.
    ...
    # one weight-shared head: Dense applies over the last axis, so every axis shares the weights.
...
def build_resqc_loss(generator, regression_model, log_min, log_span, huber_delta, degraded_weight=0.0):
    # per-axis spacing target from the generator's named layer (mm), as normalized log-spacing in [0,1].
    ...
    # weighted log-Huber: up-weight degraded axes by (1 + degraded_weight * y); 0 gives the plain mean.
    def _wloss(args):
        yt, yp = args
